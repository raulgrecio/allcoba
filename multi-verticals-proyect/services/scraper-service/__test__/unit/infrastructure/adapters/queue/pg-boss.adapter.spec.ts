...
describe('Unit: PgBossQueueAdapter', () => {
  ...
  it('debería registrar errores del bus', () => {
    const bossInstance = vi.mocked(PgBoss).mock.results[0].value;
    const errorListener = bossInstance.on.mock.calls.find(c => c[0] === 'error')[1];
  ...
  it('debería publicar con opciones', async () => {
    ...
    const bossInstance = vi.mocked(PgBoss).mock.results[0].value;
    expect(bossInstance.send).toHaveBeenCalledWith('job', {}, expect.objectContaining({ priority: 10, retryLimit: 5 }));
  });
});
