 ...
 *
 * Key facts:
 *   - sourceId = slug from /putas-valencia/{slug}/
 *   - listing requires login — only profile pages extracted
...
const mapPhoto = (photo: HotvalenciaPayload['photos'][number], idx: number): ScrapedPhoto => ({
  id: `hotvalencia:photo:${idx}`,
  url: photo.src,
  thumbnail: photo.src,
  isPrimary: idx === 0,
  isVerified: false,
...
export const mapHotvalencia = async (
  payload: HotvalenciaPayload,
  ...
  return {
    ...
    metadata: { source: HOTVALENCIA_SOURCE, adapterVersion: 'v2' },
