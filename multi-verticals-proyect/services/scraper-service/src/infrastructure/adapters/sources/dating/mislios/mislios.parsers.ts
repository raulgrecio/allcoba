/**
 * Pure parsers for mislios.com raw payload fields.
 */

/** Extract slug from URL: /anuncios/{slug}/ → slug */
export const parseSourceIdFromUrl = (url: string): string => {
  const parts = new URL(url).pathname.split('/').filter(Boolean);
  // /anuncios/{slug} → slug is last segment
  return parts[parts.length - 1] ?? url;
};

/** Normalize tel: href → 9-digit Spanish phone. */
export const parseMisliosPhone = (href: string | undefined): string | undefined => {
