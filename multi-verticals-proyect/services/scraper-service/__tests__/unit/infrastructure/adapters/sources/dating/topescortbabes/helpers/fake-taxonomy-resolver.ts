 ...
 *
 * Strategy: return a branded id derived from the slug itself, so tests can
 * assert deterministic outputs without needing a real catalog. By default
 * every lookup resolves (returns `<kind>:<slug>` cast to the branded type).
