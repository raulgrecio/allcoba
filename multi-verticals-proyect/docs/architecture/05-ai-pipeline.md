...
export class ConversationAnalyzer {
  ...
  async analyze(conversationText: string, vertical: string): Promise<CustomerTags | null> {
...
export async function processAnalyzeConversationJob(job: Job): Promise<void> {
  ...
  const encryptedConversation = await conversationRepo.getEncrypted(providerId, conversationId);
...
export class ImageModerator {
  ...
  async moderate(imageBuffer: Buffer, vertical: string): Promise<ModerationResult> {
  ...
  private async checkFaces(tensor: tf.Tensor3D, vertical: string): Promise<ModerationResult> {
